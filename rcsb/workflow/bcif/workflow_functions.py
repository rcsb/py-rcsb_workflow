...
class WorkflowUtilities:
    ...
    def __init__(self, coast:str=None, interpolation:bool=True, out:str=None, 
    ...
    def get_pdb_list(self, load_type: str) -> list:
        ...
        print("read timestamps from %s" % url)
        pdb_list = []
    ...
    def get_all_current_pdb_ids_with_timestamps(self) -> Tuple[dict, str]:
        ...
        all_pdb_ids_url = os.path.join(self.get_prerelease_ftp_file_base_url(), self.pdbIdsTimestampFilePath)
        with urllib.request.urlopen(all_pdb_ids_url) as url:
            ...
            for pdb_id in data:
                try:
                    ...
                    dat = "%s%s" % (dat, offset)
                    datetime_object = datetime.datetime.strptime(dat, "%Y-%m-%dT%H:%M:%S%z")
    ...
    def get_what_coast_we_are(self) -> str:
        ...
        if coast_path != "west" and coast_path != "east":
            raise ValueError(
    ...
    def get_all_current_model_ids_with_metadata(self) -> dict:
        try:
            ...
            holdingsFileUrl = os.path.join(self.get_csm_file_repo_base_url(), self.csmHoldingsUrl)
            dic = {}
    ...
    def get_file_repo_base_url_content_type_aware(self, content_type=ContentTypeEnum.EXPERIMENTAL.value) -> str:
        if content_type == ContentTypeEnum.EXPERIMENTAL.value:
            ...
            if content_type == ContentTypeEnum.COMPUTATIONAL.value:
                ...
                raise ValueError("Unsupported value for 'content_type' parameter: '%s'" % content_type)

    def get_prerelease_ftp_file_base_url(self) -> str:
        if self.appendCoastToFtpFileBasePath:
            return self.prereleaseFtpFileBasePath % self.get_what_coast_we_are()
        else:
            return self.prereleaseFtpFileBasePath
