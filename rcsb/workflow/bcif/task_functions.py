...
def get_pdb_list_(workflow_utility: WorkflowUtilities, load_type: str, list_file_base: str, pdb_list_filename: str, result=None) -> bool:
...
def get_csm_list_(workflow_utility: WorkflowUtilities, load_type: str, list_file_base: str, csm_list_filename: str, result=None) -> bool:
...
def make_task_list_from_remote_(list_file_base: str, pdb_list_filename: str, csm_list_filename: str,
                                input_list_filename: str, maxfiles: int, workflow_utility: WorkflowUtilities, result=None) -> bool:
    # read pdb list
    pdb_list = None
...
def make_task_list_from_local_(local_data_path: str, list_file_base: str, input_list_filename: str, result=None) -> bool:
    """
...
def split_tasks_(list_file_base: str, input_list_filename: str, input_list_2d: str, maxfiles: int, subtasks:int, result=None) -> List[int]:
    # read task list
    tasklist = None
...
def split_list(nfiles:int, subtasks:int, tasklist:List[str]) -> List[List[str]]:
    ...
    if step < 1:
        step = 1
    steps = nfiles // step
    logger.info("split list has %d files and %d steps with step %d" % (nfiles, steps, step))
...
def local_task_map_(index: int, list_file_base: str, input_list_2d: str, temp_path: str, update_base: str,
                    compress: bool, local_inputs_or_remote: str, python_molstar_java: str, batch_size: int, 
    ...
    if python_molstar_java == "python":
        ...
        try:
            ...
            logger.exception("error - failed to create dictionary api")

    def batch_task(tasks, local_inputs_or_remote, update_base, compress, temp_path, workflow_utility, python_molstar_java, da, molstar_cmd):
        for task in tasks:
            single_task(task, local_inputs_or_remote, update_base, compress, temp_path, workflow_utility, python_molstar_java, da, molstar_cmd)

    def single_task(line, local_inputs_or_remote, update_base, compress, temp_path, workflow_utility, python_molstar_java, da, molstar_cmd):
        """
        ...
        # convert to bcif
        try:
            result = bcifconvert(cif_file_path, bcif_file_path, temp_path, python_molstar_java, da, molstar_cmd)
            if not result:
    ...
    if batch_size == 1:
        ...
        for line in infiles:
            args = (line, local_inputs_or_remote, update_base, compress, temp_path, workflow_utility, python_molstar_java, da, molstar_cmd)
            single_task(*args)
    else:
        ...
        for task in tasks:
            args = (task, local_inputs_or_remote, update_base, compress, temp_path, workflow_utility, python_molstar_java, da, molstar_cmd)
...
def validate_output_(list_file_base:str, input_list_filename:str, update_base:str, compress:bool, missing_file_base:str, missing_filename:str, wu:WorkflowUtilities, result=None) -> bool:
    ...
    with open(input_list_file, "rb") as r:
        ...
        for line in data:
            ...
            content_type = wu.contentTypeDir[line.split()[2]]
